                  ...
                  ? entry.vcsBackend === "jj"
                    ? Effect.gen(function* () {
                        ...
                        const registered = workspaces.workspaces.find(
                          (workspace) => workspace.name === entry.worktreeOwnership!.token,
                        );
                        if (!existsSync(plannedWorkspacePath)) {
                          ...
                          const repository = yield* input.jj.detectRepository(entry.workspaceRoot);
                          if (!repository) {
                            return yield* Effect.fail(
                              new Error(
                          ...
                          Effect.gen(function* () {
                            ...
                            const branch = newBranch
                              ? (yield* input.git.listBranches({
                                  ...
                                  (candidate) =>
                                    !candidate.isRemote && candidate.name === newBranch,
                                )
                              : null;
                            if (!entry.worktreeOwnership) {
                            ...
                            if (
                              newBranch !== null &&
                              branch?.worktreePath !== plannedWorktreePath
                            ) {
                              return yield* Effect.fail(
                                new Error(
