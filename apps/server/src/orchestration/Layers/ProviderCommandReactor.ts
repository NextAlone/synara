...
const make = Effect.gen(function* () {
  ...
  const resolveProjectedThreadCheckpointWorkspace = Effect.fnUntraced(function* (
    ...
    return {
      cwd,
      backend,
      ...(backend === "jj" && thread.envMode === "worktree" && thread.worktreePath
  ...
  interface EditReplayWorkspaceRestorePlan extends CheckpointWorkspaceInput {
  ...
  const executeEditReplayWorkspaceRestore = Effect.fnUntraced(function* (
    ...
    const { targetTurnCount, ...workspace } = plan;
  ...
  const dispatchTurnForThread = Effect.fnUntraced(function* (input: {
    ...
    const captureMessageStartCheckpoint = Effect.gen(function* () {
      ...
      // the user's submit moment, not early provider edits. This is a strict
      // precondition: dispatching without a baseline makes Diff and Undo
      // permanently non-authoritative for the turn.
