...
async function ensureBrowserHostPipeServer(): Promise<void> {
  ...
  const server = new BrowserHostPipeServer(browserManager, {
    ...
    requestOpenPanel: (threadId) => {
      if (!threadId) return;
      sendDesktopRendererIpc(mainWindow?.webContents, IPC.browser.requestOpenPanel, { threadId });
    },
  });
  await server.start();
...
function startBackend(trigger: BackendStartTrigger = "lifecycle"): void {
  ...
  const child = ChildProcess.spawn(process.execPath, [...backendNodeArgs(), backendEntry], {
    ...
    // are observable even when packaged log setup is unavailable. FD 3 carries
    // the private browser-host capability; the IPC channel closes if Electron
    // dies, allowing the backend to release its database lock.
