...
import { resolveDraftThreadBranchForEnvironmentMode } from "../lib/jjWorkspaceBase";
import {
...
export default function ChatView({
  ...
  const onInterruptFromStopControl = useCallback(() => {
    void interruptCurrentTurn().catch((error: unknown) => {
      toastManager.add({
        type: "error",
  ...
  const onSend = async (
    ...
    const nextRuntimeModeForSend = runtimeModeForSend;
  ...
  const onEnvModeChange = useCallback(
    (mode: DraftThreadEnvMode) => {
      ...
      // JJ Worktree reuses the project's last explicit base before falling back to `@`.
